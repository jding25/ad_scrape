...
productcard = pd.read_csv('productcard.csv')
...
work_data.to_csv('productcard_work.csv',index = False)



for i in range(productcard.shape[1]):
    print(i)
#     #modify links
    link = productcard.columns[i].replace("'","").split("?")[0]
    curr_html = scrollPage(link)
    curr = get_commenturl_likes_views(link, curr_html)
    with open('productcard_work.csv', 'a') as f_object:
