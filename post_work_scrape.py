...
postcard = pd.read_csv('postcard.csv')
...
work_data.to_csv('postcard_work.csv',index = False)



for i in range(postcard.shape[1]):
    print(i)
#     #modify links
    link = postcard.columns[i].replace("'","").split("?")[0]
    curr_html = scrollPage(link)
    curr = get_commenturl_likes_views(link, curr_html)
    with open('postcard_work.csv', 'a') as f_object:
