import pandas as pd
businesscard = pd.read_csv('businesscard.csv')
...
# work_data.to_csv('businesscard_work.csv',index = False)



for i in range(businesscard.shape[1]):
    print(i)
#     #modify links
    link = businesscard.columns[i].replace("'","").split("?")[0]
    curr_html = scrollPage(link)
    curr = get_commenturl_likes_views(link, curr_html)
    with open('businesscard_work.csv', 'a') as f_object:
