...
work_data.to_csv('creditcard_work.csv',index = False)



for i in range(creditcard.shape[1]):
    print(i)
#     #modify links
    link = creditcard.columns[i].replace("'","").split("?")[0]
    curr_html = scrollPage(link)
    curr = get_commenturl_likes_views(link, curr_html)
    with open('creditcard_work.csv', 'a') as f_object:
